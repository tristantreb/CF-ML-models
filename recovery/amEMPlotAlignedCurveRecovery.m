...
xl = [-0.5, ((max_offset + align_wind) - 0.5)]; % intervention starts at day 0
...
[xl, yl] = plotLatentCurveRecovery(ax, max_offset, align_wind, min_offset, (mprofile_pre), xl, yl, 'red', ':', 0.5, anchor);
%[xl, yl] = plotLatentCurve(ax, max_offset, align_wind, min_offset, smooth(mprofile_pre, 5), xl, yl, 'red', '-', 0.5, anchor);
[xl, yl] = plotLatentCurveRecovery(ax, max_offset, align_wind, min_offset, movmean(mprofile_pre, 3, 'omitnan'), xl, yl, 'red', '-', 0.5, anchor);
[xl, yl] = plotLatentCurveRecovery(ax, max_offset, align_wind, min_offset, (mmeancurvemean), xl, yl, 'blue', ':', 0.5, anchor);
%[xl, yl] = plotLatentCurve(ax, max_offset, align_wind, min_offset, smooth(mmeancurvemean, 5), xl, yl, 'blue', '-', 0.5, anchor);
[xl, yl] = plotLatentCurveRecovery(ax, max_offset, align_wind, min_offset, movmean(mmeancurvemean, 3, 'omitnan'), xl, yl, 'blue', '-', 0.5, anchor);

if sigmamethod == 4
    [xl, yl] = plotLatentCurveRecovery(ax, max_offset, align_wind, min_offset, (mmeancurvemean + mmeancurvestd), xl, yl, 'blue', ':', 0.5, anchor);
%   [xl, yl] = plotLatentCurve(ax, max_offset, align_wind, min_offset, smooth(mmeancurvemean + mmeancurvestd, 5), xl, yl, 'blue', '--', 0.5, anchor);
    [xl, yl] = plotLatentCurveRecovery(ax, max_offset, align_wind, min_offset, movmean(mmeancurvemean + mmeancurvestd, 3, 'omitnan'), xl, yl, 'blue', '--', 0.5, anchor);
    [xl, yl] = plotLatentCurveRecovery(ax, max_offset, align_wind, min_offset, (mmeancurvemean - mmeancurvestd), xl, yl, 'blue', ':', 0.5, anchor);
%   [xl, yl] = plotLatentCurve(ax, max_offset, align_wind, min_offset, smooth(mmeancurvemean - mmeancurvestd, 5), xl, yl, 'blue', '--', 0.5, anchor);
    [xl, yl] = plotLatentCurveRecovery(ax, max_offset, align_wind, min_offset, movmean(mmeancurvemean - mmeancurvestd, 3, 'omitnan'), xl, yl, 'blue', '--', 0.5, anchor);
end
...
if isequal(run_type,'Best Alignment')
    bar( 1:(max_offset + align_wind - 1), max_points, 1.0, 'EdgeColor', 'none', 'FaceColor', [0.3, 0.3, 0.3], 'FaceAlpha', 0.4);
end
hold on;
bar( 1:(max_offset + align_wind - 1), mmeancurvecount, 1.0, 'EdgeColor', 'none', 'FaceColor', 'black', 'FaceAlpha', 0.5, 'LineWidth', 0.2);
hold off;
