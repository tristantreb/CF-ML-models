% creates list of oral & IV treatments for recovery study
%
...
for i = 0:numdays-1
    ivandmeasurestable.Properties.VariableNames{i + nkeycols + 2} = sprintf('StartPlus%d',i);
end
...
% idx contains all values in [start, start+numdays[;
...
for a = 0:numdays-1
    colname = sprintf('StartPlus%d', a);
    dayidx = (pdcountmtable.SmartCareID == ivTreatments.ID(i)) & (pdcountmtable.DateNum == ivTreatments.IVDateNum(i) + a);
    if sum(dayidx,1) > 0
...
for i = 2:size(ivandmeasurestable, 1)
    ...
    if (ivandmeasurestable.SmartCareID(i) == ivandmeasurestable.SmartCareID(i-1) ...
                && ivandmeasurestable.IVDateNum(i) - ivandmeasurestable.IVStopDateNum(i-1) < (numdays + treatgap))
...
outputfilename = sprintf('%sivandmeasures_recovery_gap%d.mat', study, treatgap);
...
outputfilename = sprintf('%sMeasuresPostTreatments_gap%d.xlsx', study, treatgap);
