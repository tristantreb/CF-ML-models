function [wcxpvaltable] = amEMMCPlotVariablesVsLatentCurveSetForPaperRecovery(amInterventions, pmPatients, pmPatientMeasStats, ivandmeasurestable, ...
...
scattervartext = {'Stable FEV1',            'Box'; ...
                'BMI',                      'Box';  ...
                'Age',                      'Box';  ...
                'Time to treatment response', 'Box'; ...
                'Day of Year',              'Scatter'; ...
                'CRP Adm',                  'Box'; ...
                'CRP Stable',               'Box'};            
...
% add time to response to am Interventions
lc = innerjoin(lc, amInterventions, 'LeftKeys', {'SmartCareID', 'IVScaledDateNum'}, 'RightKeys', {'SmartCareID', 'IVScaledDateNum'}, 'RightVariables', {'TimeToResponse'});
scattervardata(:, 4) = lc.TimeToResponse;
...
for v = 1:nscattervars
    ...
    for i = 1:nlatentcurves
        ...
        [pval, h] = ranksum(scattervardata(lc.LatentCurve == i, v), scattervardata(lc.LatentCurve ~= i, v),0.01);
...
for v = 1:nbarvars
    ...
    elseif ismember(barvartext(v), {'Nbr of Interventions', 'Pct Nbr of Interventions'})
        % amInterventions.Pred   = amInterventions.IVScaledDateNum + amInterventions.Offset
    ...
    elseif ismember(barvartext(v), {'Time Since Last Ex', 'Time Since Last Ex or Study Start', 'Pct Time Since Last Ex', 'Pct Time Since Last Ex or Study Start'})
        ...
        for i = 1:size(blc, 1)
            ...
            previdx = find(tmpivandmtable.SmartCareID == scid & tmpivandmtable.IVScaledStopDateNum < blc.IVScaledDateNum(i), 1, 'last');
            if size(previdx, 1) ~= 0
                blc.LastTreatEnd(i)       = tmpivandmtable.IVScaledStopDateNum(previdx);
                blc.TimeSinceLastTreat(i) = blc.IVScaledDateNum(i) - blc.LastTreatEnd(i);
                blc.Category(i)           = 1;
            else 
                blc.TimeSinceLastTreat(i) = blc.IVScaledDateNum(i);
                blc.Category(i)           = 2;
            end
        end
    ...
    for i = 1:nlatentcurves
        ...
        [h, pval, ~] = chi2gof(uniquecats, 'freq', freqtable.ObsFreq', 'expected', freqtable.ExpFreq', 'ctrs', uniquecats,'Alpha', 0.01);
