function amEMMCPlotSuperimposedAlignedCurvesForPaper3Recovery(meancurvemean, meancurvecount, amIntrNormcube, amInterventions, normmean, normstd, ...
    measures, offset, align_wind, nmeasures, run_type, ex_start, plotname, plotsubfolder, nlatentcurves, ...
...
for n = 1:nlatentcurves
    ...
    for m = 1:nmeasures
        ...
        meancurvemean(n, meancurvecount(n, :, m) < countthreshold, m) = NaN;
        if shiftmode == 1
            vertshift = mean(meancurvemean(n, 1:(align_wind + offset.span + ex_start(n)), m));
        elseif shiftmode == 2
            vertshift = max(meancurvemean(n, 1:(align_wind + offset.span + ex_start(n)), m));
        elseif shiftmode == 3
            vertshift = meancurvemean(n, (align_wind + offset.span + ex_start(n)), m);
        elseif shiftmode == 4
            if (align_wind + offset.span + ex_start(n) - meanwindow) < 1
                shiftfrom = 1;
            else
                shiftfrom = (align_wind + offset.span + ex_start(n) - meanwindow);
            end 
            vertshift = mean(meancurvemean(n, shiftfrom:(align_wind + offset.span + ex_start(n)), m));
        end
...
for i = 1:(ntitles + nlcrow + nmeasrows + nlabels)
    ...
    elseif type == 6 || type == 8
        ...
        for n = 1:nlatentcurves
            lc = lcsort(n);
            xto = (align_wind + offset.span - 1 + ex_start(lc));
            xfrom   = (1 + ex_start(lc));
            xl = [xfrom, xto];
            tmp_meancurvemean  = reshape(meancurvemean(lc, :, :),  [align_wind + offset.span - 1, nmeasures]);
            ...
            if tmp_ninterventions ~= 0
                ...
                if type == 6
                    ...
                    plotSuperimposedAlignedCurvesForPaper(ax, tmp_meancurvemean, xl, yl, ...
                            tmpmeasures, tmpnmeasures, offset, align_wind, ex_start(lc), study);
                elseif type == 8
                    plotSuperimposedAlignedCurvesForPaper(ax, tmp_meancurvemean, xl, yl, ...
                            tmpsubsetmeasures, tmpnsubsetmeasures, offset, align_wind, ex_start(lc), study);
    ...
    elseif type == 5 || type == 7
        if (examplemode ~= 0)
            ...
            for n = 1:nlatentcurves
                ...
                if all(isnan(amnormcubesingleintrsmth(1, 1:(align_wind + offset.span + tmp_ex_start + tmp_offset), 1)))
                        vertshift = 0;
                else
                    if shiftmode == 1
                        vertshift = mean(amnormcubesingleintrsmth(1, 1:(align_wind + offset.span + tmp_ex_start + tmp_offset), 1), 'omitnan');
                    elseif shiftmode == 2
                        vertshift = max(amnormcubesingleintrsmth(1, 1:(align_wind + offset.span + tmp_ex_start + tmp_offset), 1), 'omitnan');
                    elseif shiftmode == 3
                        vertshift = amnormcubesingleintrsmth(1, find(~isnan(amnormcubesingleintrsmth(1, 1:(align_wind + offset.span + tmp_ex_start + tmp_offset), 1)), 1, 'last'), 1);
                    elseif shiftmode == 4
                        if (align_wind + offset.span + tmp_ex_start + tmp_offset - meanwindow) < 1
                            shiftfrom = 1;
                        else
                            shiftfrom = (align_wind + offset.span + tmp_ex_start + tmp_offset - meanwindow);
                        end 
                        vertshift = mean(amnormcubesingleintrsmth(1, shiftfrom:(align_wind + offset.span + tmp_ex_start + tmp_offset), 1), 'omitnan');
                    end
                end
                ...
                lwidth = 1.5;
                days = xl(1):xl(2);
                dto = align_wind + offset.up - 1 - tmp_offset;
                dfrom = offset.down + 1 + tmp_offset;
